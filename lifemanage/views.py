...
#入力ページ
class FirstInputView(LoginRequiredMixin, FirstForm, CreateView):
    model = Person
    template_name = 'pages/firstinput.html'
    form = FirstForm
    success_url = reverse_lazy('lifemanage:list')
    
    def form_valid(self, form):
        person = form.save(commit=False)
        person.user = self.request.user
        person.save()
        messages.success(self.request, '基本情報を入力しました')
        return super().form_valid(form)

    def form_invalid(self, form):
        messages.error(self.request, '基本情報が入力できませんでした')
...
class IndexUpdateView(UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy('lifemanage:detail', kwargs={'pk':self.kwargs['pk']})
